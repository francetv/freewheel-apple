...
typedef NS_ENUM(NSUInteger, FWVideoAssetAutoPlayType) {
	...
	/** Video asset auto play type: ClickToPlay */
	FWVideoAssetAutoPlayTypeClickToPlay = 3
};
...
/**
	Parameter: skip button width in pixels.
	...
	- Note: This parameter should be set before slot starts, otherwise it will have no effect.
	- Note: Ad traffickers should use `extension.skippableAd.width` instead of `FWParameterExtensionSkippableAdButtonHeight` when setting this parameter in MRM UI.
	- See:
		- `[FWContext setParameter:withValue:forLevel:]`
		- `[FWSlot setParameter:withValue:]`
		- `[FWCreativeRendition setParameter:withValue:]`
 */
FW_EXTERN NSString *const FWParameterExtensionSkippableAdButtonWidth;

/**
	Parameter: skip button height in pixels.
	...
	- Note: This parameter should be set before slot starts, otherwise it will have no effect.
	- Note: Ad traffickers should use `extension.skippableAd.height` instead of `FWParameterExtensionSkippableAdButtonHeight` when setting this parameter in MRM UI.
	- See:
		- `[FWContext setParameter:withValue:forLevel:]`
		- `[FWSlot setParameter:withValue:]`
		- `[FWCreativeRendition setParameter:withValue:]`
 */
FW_EXTERN NSString *const FWParameterExtensionSkippableAdButtonHeight;

/**
	Parameter: skip button position inside video display base.
	Valid values:
		- `@"bottom-left"`: bottom-left corner of video display base
		- `@"bottom-center"`: bottom-center corner of video display base
		- `@"bottom-right"`: bottom-right corner of video display base
		- `@"top-left"`: top-left corner of video display base
		- `@"top-center"`: top-center corner of video display base
		- `@"top-right"`: top-right corner of video display base

	Default value: `@"bottom-right"`

	- Note: This parameter should be set before slot starts, otherwise it will have no effect.
	- Note: Ad traffickers should use `extension.skippableAd.position` instead of `FWParameterExtensionSkippableAdButtonPosition` when setting this parameter in MRM UI.
	- See:
		- `[FWContext setParameter:withValue:forLevel:]`
		- `[FWSlot setParameter:withValue:]`
		- `[FWCreativeRendition setParameter:withValue:]`
 */
FW_EXTERN NSString *const FWParameterExtensionSkippableAdButtonPosition;

/**
	Parameter: skip button HTML.
	The value of the parameter is a HTML snippet that renders a button inside the inAppView. The HTML must contain an element with with the id "_fw_skip_button", that the extension will use the display the countdown and button text.
 ...
 <button id='_fw_skip_button' style='background-color:#01020185; border:none; padding:8px; color:white; font-size:16px; text-align:center; text-decoration:none'> \
	...
	- Note: Ad traffickers should use `extension.skippableAd.html` instead of `FWParameterExtensionSkippableAdButtonHtml` when setting this parameter in MRM UI.
	- See:
		- `[FWContext setParameter:withValue:forLevel:]`
		- `[FWSlot setParameter:withValue:]`
		- `[FWCreativeRendition setParameter:withValue:]`
 ...
 */
FW_EXTERN NSString *const FWParameterExtensionSkippableAdButtonHtml;

/**
	Parameter: skippable ad button text.

	Valid values: NSString

	Default value: `@"Skip Ad"`

	- Note: This parameter should be set before slot starts, otherwise it will have no effect.
	- Note: Ad traffickers should use `extension.skippableAd.text` instead of `FWParamaterExtensionSkippableAdButtonText` when setting this parameter in MRM UI.
	- See:
		- `[FWContext setParameter:withValue:forLevel:]`
		- `[FWSlot setParameter:withValue:]`
		- `[FWCreativeRendition setParameter:withValue:]`
 */
FW_EXTERN NSString *const FWParamaterExtensionSkippableAdButtonText;

/**
	Parameter: Enable / disable counting replay callback.

	Valid values: NSString `@"YES"`(enable), `@"NO"`(disable)

	Default value: `@"NO"`

	- Note: If the value is set to "true", init value will be included in quartile callback.
 */
FW_EXTERN NSString *const FWParameterEnableCountingReplayCallback;
...
/**
 Parameter: Used to set GPP CMD API String. This key should be set in [NSUserDefaults standardUserDefaults].

 Valid values: NSString representing the GPP CMD API Full consent string in its encoded form.

 Default value: `nil`

 - Note:
     - For AdManager to automatically retrieve the GPP String, this parameter should be set before initializing FWRequestConfiguration. If the value needs to be set after initializing FWRequestConfiguration, please assign FWRequestConfiguration's consentConfiguration to a new FWConsentConfiguration object. Changes made to consentConfiguration after an AdRequest calls submitWithTimeout will be ignored.
     - Values will be set to nil if they are not an instance of NSString or an instance of a class that inherits from NSString.
 - See:
     - `[FWRequestConfiguration setConsentConfiguration:]`
     - `[FWAdRequest submitWithTimeout:]`
     - `FWConsentConfiguration`
     - `FWGPPCMPAPIConsentConfiguration`
*/
FW_EXTERN NSString *const FWParameterGPPCMPAPIStringKey;

/**
    Parameter: Used to set GPP CMD API Section ID(s). This key should be set in [NSUserDefaults standardUserDefaults].

    Valid values: NSString representing the GPP CMD API Section ID(s) considered to be in force. Multiple IDs are separated by underscore, e.g. “2_3”

    Default value: `nil`

    - Note:
        - For AdManager to automatically retrieve the GPP CMD API Section ID(s), this parameter should be set before initializing FWRequestConfiguration. If the value needs to be set after initializing FWRequestConfiguration, please assign FWRequestConfiguration's consentConfiguration to a new FWConsentConfiguration object. Changes made to consentConfiguration after an AdRequest calls submitWithTimeout will be ignored.
        - Values will be set to nil if they are not an instance of NSString or an instance of a class that inherits from NSString.
    - See:
        - `[FWRequestConfiguration setConsentConfiguration:]`
        - `[FWAdRequest submitWithTimeout:]`
        ...
        - `FWGPPCMPAPIConsentConfiguration`
*/
FW_EXTERN NSString *const FWParameterGPPCMPAPISectionIdStringKey;
...
/**
	Use this key in the `details` dictionary to provide detailed error info when the renderer/translator transits to `FWRendererStateStarted`. Expects NSInteger.
 
	Example usage:
	```[rendererController handleStateTransition:FWRendererStateStarted info:[FWInfoKeyStartTimePosition:@"1"]];```
 */
FW_EXTERN NSString *const FWInfoKeyStartTimePosition;
